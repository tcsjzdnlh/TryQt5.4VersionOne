                                     ...
                                     QPainter *painter, const QWidget *widget) const
{
    ...
    case CC_Slider:
        if (const QStyleOptionSlider *slider = qstyleoption_cast<const QStyleOptionSlider *>(option)) 
		{
			...
			//QString strLog;
			//
			//const QMetaObject *pMeta = metaObject();
			//int index = pMeta->indexOfEnumerator( "PixelMetric" );
			//QMetaEnum me = pMeta->enumerator( index );


			////iValue = pixelMetric( PM_TickMarkOffset, slider, widget );
			//// 1.
			//strKey = me.key( PM_SliderControlThickness );
			//iValue = pixelMetric( PM_SliderControlThickness, slider, widget );
			//
			//strLog = QString( "%1 value is : %2 " ).arg( strKey ).arg( iValue );
			//g_pWnd->Log( strLog );

			//// 2.
			//strKey = me.key( PM_SliderLength );
			//iValue = pixelMetric( PM_SliderLength, slider, widget );

			//strLog = QString( "%1 value is : %2 " ).arg( strKey ).arg( iValue );
			//g_pWnd->Log( strLog );

			//// 3.
			//strKey = me.key( PM_SliderSpaceAvailable );
			//iValue = pixelMetric( PM_SliderSpaceAvailable, slider, widget );

			//strLog = QString( "%1 value is : %2 " ).arg( strKey ).arg( iValue );
			//g_pWnd->Log( strLog );

			//// 4.
			//strKey = me.key( PM_SliderTickmarkOffset );
			//iValue = pixelMetric( PM_SliderTickmarkOffset, slider, widget );
			//
			//strLog = QString( "%1 value is : %2 " ).arg( strKey ).arg( iValue );
			//g_pWnd->Log( strLog );
    ...
    case CC_GroupBox:
                ...
                = qstyleoption_cast<const QStyleOptionGroupBox *>(option)) 
		{
            ...
            if (groupBox->subControls & SC_GroupBoxLabel) {
                ...
                painter->drawText(tileRect.translated(0, 1), Qt::AlignVCenter | Qt::AlignHCenter, groupBox->text);
                painter->drawText(tileRect.translated(2, 1), Qt::AlignVCenter | Qt::AlignHCenter, groupBox->text);
                
				painter->setPen(QColor(0, 0, 0, opacity * 2));
                painter->drawText(tileRect.translated(1, 1), Qt::AlignVCenter | Qt::AlignHCenter, groupBox->text);
...
void ArthurStyle::polish(QWidget *widget)
{
    ...
    if (widget->isWindow()) 
	{
        pal.setColor(QPalette::Background, QColor(241, 241, 241 /*255, 0, 0*/));
